...
export default function FeaturedBooksSection() {
  ...
  const [filteredBooks, setFilteredBooks] = useState(placeholderBooks);
  
  // Aplicar filtro cuando cambia el género activo
  useEffect(() => {
    if (activeGenre === 'Todos') {
      setFilteredBooks(placeholderBooks);
    } else {
      setFilteredBooks(placeholderBooks.filter(book => book.genre === activeGenre));
    }
  }, [activeGenre]);
  return (
    <section id="libros" className="w-full py-20 md:py-32 relative overflow-hidden">
      ...
      {/* Textura de cuero vintage */}
      <div className="absolute inset-0 bg-[url('/images/leather-texture.jpg')] bg-cover opacity-[0.07] mix-blend-overlay z-0"></div>
        ...
        <div className="relative">
          ...
          <div className="relative grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 z-10">
            {filteredBooks.map((book) => (
              <div key={book.id} className="group transform hover:scale-105 transition-all duration-500">
